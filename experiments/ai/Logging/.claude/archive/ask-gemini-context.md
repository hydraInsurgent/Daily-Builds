# Context for Review: Plan.md (Logging in .NET Core Implementation Plan)
...
- [ ] 🟥 **Step 2: Console + configuration**
  ...
  - [ ] Registering logging in DI (`AddLogging`, host builder)
  - [ ] Console provider
  - [ ] `appsettings.json` "Logging" section; default minimum level and category (namespace) overrides
  - [ ] Log level enum and extension methods (`LogInformation`, `LogWarning`, `LogError`)
  - [ ] Where output appears (console)
  - [ ] Isolation: folder/namespace for this variation; minimal host + one service that logs
